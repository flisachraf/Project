...
const ArticleList = ({ articles, view, onPaginationChange, currentPage, totalPages, fetchArticles, openEditModal }) => {
  const token = localStorage.getItem('token');
  const { user } = useAuth();
  const [viewNews, setViewNews] = useState(false);
  const [editNews, setEditNews] = useState(false);
  const [selectedNew, setSelectedNew] = useState({});
  console.log("************ list of news from edit articlesList ",articles)
  ...
  return (
    <div className="p-4 lg:p-8 bg-white border border-gray-200 rounded-lg shadow-lg">
      ...
      <Modal
        isOpen={editNews}
        onRequestClose={closeEditModal}
      > {selectedNew.category === "Infographs" |selectedNew.category==="VideoGraphs" ? <EditMediaItem mediaItem={selectedNew} user={user} closeEditMedia={closeEditModal} />:<EditArticle article={selectedNew} closeEditModal={closeEditModal} user={user} />}
